...
internal static class CrockfordEncoding
{
    ...
    /// <summary>
    /// Encodes the specified unsigned 64-bit integer into a Crockford Base-32 representation and writes the result into
    /// the provided byte span.
    /// </summary>
    /// <remarks>
    /// This method assumes the caller provides a destination buffer of at least 13 bytes.
    /// The encoded digits are written from left to right starting at index 0.
    /// </remarks>
    /// <param name="value">The unsigned 64-bit integer value to encode.</param>
    /// <param name="destination">The span of bytes that receives the encoded representation. Must be at least 13 bytes.</param>
    ...
    public static int Encode(ulong value, Span<byte> destination)
    {
        ...
        // Calculate required length: ceil((64 - leadingZeros) / 5)
        var requiredLength = (64 - BitOperations.LeadingZeroCount(value) + 4) / 5;
    ...
    /// <summary>
    /// Decodes a sequence of bytes encoded in Crockford 32 bit encoding format into an unsigned 64-bit integer.
    /// </summary>
    /// <remarks>The method processes each byte in the input as a 5-bit value and combines them into a single
    /// integer. The input must not contain more than 13 bytes, as only the lowest 65 bits are used; excess bytes may
    ...
    /// <returns>The unsigned 64-bit integer obtained by decoding the input byte sequence.</returns>
    /// <exception cref="ArgumentException">Thrown when the input span exceeds 13 bytes.</exception>
    ...
    public static ulong Decode(ReadOnlySpan<byte> bytes)
    {
        ...
        // Maximum 13 characters can encode 65 bits (13 * 5 = 65)
        // Since we're decoding to ulong (64 bits), 13 is the safe maximum
        if (bytes.Length > 13)
    ...
    private static void ThrowInputTooLarge(int length) =>
        throw new ArgumentException($"Input too large. Maximum 13 bytes allowed for decoding, but got {length} bytes.");
}
